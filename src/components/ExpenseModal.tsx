...
export const ExpenseModal: React.FC<ExpenseModalProps> = ({
  ...
  // confirmation ('voice' | 'receipt') until the user edits or taps Redo.
  const [aiFilledFrom, setAiFilledFrom] = React.useState<null | 'voice' | 'receipt'>(null);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* QUICK ADD HERO — voice + scan, made a first-class feature */}
          <style>{`
            @keyframes qa-halo { 0%{transform:scale(0.8);opacity:0.6} 70%{transform:scale(1.9);opacity:0} 100%{opacity:0} }
          ...
          {aiFilledFrom ? (
            <div style={{ background: '#ECFDF5', border: '1px solid #A7F3D0', borderRadius: '12px', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: '9px' }}>
              ...
              <span style={{ fontSize: '12.5px', fontWeight: 600, color: '#065F46' }}>
                Filled from {aiFilledFrom === 'voice' ? 'voice' : 'receipt'}
              </span>
              <button
                type="button"
                onClick={() => {
                  const src = aiFilledFrom;
                  setAiFilledFrom(null);
                  if (src === 'voice') { if (!isListening) startListening(); }
                  else { openScanner(); }
                }}
                style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: '5px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600, color: '#047857', padding: 0 }}
              >
              ...
              <div style={{ marginLeft: 'auto', display: 'flex', gap: '7px' }}>
                {hasSupport && (
                  <button
                    ...
                    style={{ width: '30px', height: '30px', borderRadius: '8px', background: '#94A3B8', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  >
                ...
                <button
                  ...
                  style={{ width: '30px', height: '30px', borderRadius: '8px', background: '#34D399', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ width: '16px', height: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '12px', height: '48px' }} ref={descriptionContainerRef}>
              ...
              <div className="premium-input-wrapper" style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', right: '6px', top: '50%', transform: 'translateY(-50%)', display: 'flex', alignItems: 'center' }}>
                  <button
                    type="button"
                    onClick={() => (attachments.length > 0 ? setShowAttachmentsPreview(true) : setShowAttachMenu(true))}
                    style={{
                      position: 'relative',
                      border: 'none',
                      background: 'transparent',
                      ...
                      justifyContent: 'center',
                      padding: '4px',
                      color: attachments.length > 0 ? '#16A34A' : 'var(--t)',
                      opacity: attachments.length > 0 ? 1 : 0.7,
                    }}
                    title={attachments.length > 0 ? `${attachments.length} attachment${attachments.length > 1 ? 's' : ''} — tap to view` : 'Add attachment'}
                  >
