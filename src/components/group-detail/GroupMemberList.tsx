...
interface GroupMemberListProps {
  ...
  onSetMemberEmail?: (memberName: string, email: string) => void | Promise<void>;
}
...
export const GroupMemberList: React.FC<GroupMemberListProps> = ({
  ...
  React.useEffect(() => {
    ...
    const onPop = () => { setIsAddingInline(false); };
  ...
  // Open the inline add-friend form (from the row or the floating button).
  const openAddFriend = () => {
  ...
  const AddFriendRow: React.FC<{ bottomBorder?: boolean }> = ({ bottomBorder }) => {
    ...
    return (
      ...
      >
        <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#F97316', color: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', fontWeight: 600, flex: 'none' }}>+</div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <FullScreenAddFriend
          isOpen={isAddingInline}
          onClose={() => {
            setIsAddingInline(false);
            window.history.back();
          }}
          onAddFriends={(friends) => {
            commitSelected(friends);
            window.history.back();
          }}
        ...
        {/* Floating add-friend button (all tabs) */}
        {!isAddingInline && (
          <button
            type="button"
            ...
            onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.06)'; e.currentTarget.style.background = '#EA580C'; }}
